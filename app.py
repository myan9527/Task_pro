...
from src.core.webcore import add_routes,add_static
...
def init_jinja(app,**kw):
    logging.info('Start initializing jinja2...')
    options = dict(
...
def datetime_filter(t):
    ...
    if delta < 86400:
        return u'%s hour age' % (delta // 3600)
    if delta < 604800:
...
async def init(loop):
    await create_pool(loop=loop, host='127.0.0.1', port=3306, user='root', password='root', database='task')
    app = web.Application(loop=loop, middlewares=[
        logger_factory, response_factory
    ])
    init_jinja(app, filters=dict(datetime=datetime_filter))
    add_routes(app, 'src.views')
    add_static(app)
